      ...
      <div className="w-full sm:w-[27%] h-1/4 sm:h-full flex flex-row sm:flex-col justify-between items-start gap-2">
        {images.length > 2
          ? images.map((image, key) => (
              <div
                className={`w-full h-full sm:h-1/3 cursor-pointer rounded-2xl overflow-hidden bg-gray-200 ${
                  images.length > 2 ? `` : `animate-pulse bg-slate-300`
                } `}
              >
                <img
                  src={image}
                  alt="img"
                  id={key}
                  className={`w-full h-full cursor-pointer rounded-2xl object-cover object-center bg-gray-200 ${
          ...
          : Array.from({ length: 3 }, (_, index) => (
              <div
                className={`w-full h-full sm:h-1/3 cursor-pointer rounded-2xl overflow-hidden animate-pulse bg-slate-300`}
                key={index}
              ></div>
            ))}
      </div>
      <div
        className={`w-full h-3/4 sm:w-[73%] sm:h-full rounded-2xl ${
          images.length > 2 ? `` : `animate-pulse bg-slate-300`
        }`}
      >
        <img
